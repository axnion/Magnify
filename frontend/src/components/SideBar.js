...
const SideBar = ({
  ...
  <SideBarDivider className="sidebar"> {
      ...
      (username === null || role === 'consumer') ? undefined :
      <SideBarItem >
        <Link
          to="/addProduct"
          onClick={updateUnseen}
    ...
    <SideBarItem >
      ...
      (role === 'companyRep' || role === 'companyAdmin') && unseenThreads.length > 0 ?
        <Badge
          badgeContent={unseenThreads.length}
          secondary
          style={{ padding: '0px 24px 18px 12px' }}
        /> : undefined }
    </SideBarItem >
    ...
    <SideBarItem >
      ...
      > Log out
        ...
        username === null ?
          <SideBarItem >
            <Link to="/register" > <button className="sidebar-btn" > Create an account </button></Link >
          </SideBarItem> : undefined
      }
  </SideBarDivider>
);
